...
Each role has a specific set of permissions and constraints, sharing a common environment, or memory. For example, an Architect agent looks at a Product Requirement Document (PRD) created by a Product Manager and produces a technical design. An Engineer then uses this design to write code. This organized approach helps reduce the "hallucination" and confusion often seen when single LLMs take on large projects.
...
This Pull request intended to transform the whole repository to single and structured markdown file. This repo has also added a test file to verify whether the markdown generation and repository traversal work as expected. The core change is the implementation of a new Python module, `metagpt/utils/repo_to_markdown.py`, which contains the `RepoToMarkDown` class.
...
2.  ✓ **Noise Filtering:** The implementation must use a predefined "ignore list," such as `.git`, `__pycache__`, `node_modules`, and `.png`, to exclude binary and metadata files from the final output string.  
...
4.  ✓ **Language Detection:** The system must automatically detect the programming language based on the file extension. For example, it maps `.py` to python and `.js` to javascript, applying the correct language tag to the Markdown code block.  
5.  ✓ **Robustness:** The function must return a valid string even if the repository is empty or contains only ignored files. It should not crash the main application thread.
...
* Verify that the returned string includes the Python code.
* Verify that the returned string does not include the git data or the image data.
